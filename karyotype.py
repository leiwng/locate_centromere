...
class Karyotype:
    ...
    def __init__(self, karyotype_img_fp):
        ...
        (self.case_id, self.pic_id, self.img["type"], self.img["fext"]) = self.img["fname"].split(".")
    ...
    def _id_info(self):
        ...
        id_cntr_dicts = [
            cntr_dict
            for cntr_dict in self.cntr_dicts
            if cntr_dict["area"] < MAX_ID_CHAR_AREA and cntr_dict["area"] > MIN_ID_CHAR_AREA
        ]
        ...
        for given_key, given_cntr_dicts in id_cntr_dicts_orgby_cy.items():
            ...
            for chk_key, chk_cntr_dicts in id_cntr_dicts_orgby_cy.items():
                ...
                if (
                    given_key != chk_key
                    and abs(given_key - chk_key) <= ID_CHAR_Y_TOLERANCE
                    and chk_key not in merged_keys
                ):
        ...
        merged_id_cntr_dicts_orgby_cy = {
            key: merged_id_cntr_dicts_orgby_cy[key]
            for key in merged_id_cntr_dicts_orgby_cy
            if len(merged_id_cntr_dicts_orgby_cy[key]) >= ROW_ID_CHAR_MIN_NUM
            and len(merged_id_cntr_dicts_orgby_cy[key]) <= ROW_ID_CHAR_MAX_NUM
        }
        ...
        # 按照cy坐标从小到大排序
        merged_id_cntr_dicts_orgby_cy = dict(sorted(merged_id_cntr_dicts_orgby_cy.items(), key=lambda item: item[0]))

        # 每列的轮廓按x坐标从小到大排序
        for key in merged_id_cntr_dicts_orgby_cy:
            merged_id_cntr_dicts_orgby_cy[key] = sorted(merged_id_cntr_dicts_orgby_cy[key], key=lambda item: item["cx"])

        # SAVE RESULT to CLASS INSTANCE MEMBER PROPERTY
        self.id_char_cntr_dicts_orgby_cy = merged_id_cntr_dicts_orgby_cy
        self.id_char_cntr_dicts = [merged_id_cntr_dicts_orgby_cy[key] for key in merged_id_cntr_dicts_orgby_cy]
        ...
        for idx, key in enumerate(merged_id_cntr_dicts_orgby_cy):
            if len(merged_id_cntr_dicts_orgby_cy[key]) != id_num_in_rows[idx]:
                print(
                    f'{self.img["fname"]}: 第{idx+1}排染色体编号数量为{len(merged_id_cntr_dicts_orgby_cy[key])},应该为{id_num_in_rows[idx]}'
                )
        ...
        self.id_cntr_dicts = [merged_id_cntr_dicts_orgby_cy[key] for key in merged_id_cntr_dicts_orgby_cy]
    ...
    def read_karyotype(self):
        ...
        self.cntr_dicts = [
            cntr
            for cntr in self.cntr_dicts
            if (
                cntr["cx"] > MAX_CNTR_DISTANCE_TO_BORDER
                and cntr["cx"] < self.img["width"] - MAX_CNTR_DISTANCE_TO_BORDER
            )
            and (
                cntr["cy"] > MAX_CNTR_DISTANCE_TO_BORDER
                and cntr["cy"] < self.img["height"] - MAX_CNTR_DISTANCE_TO_BORDER
            )
        ]
        ...
        for id_char_cntrs in self.id_char_cntr_dicts:
            id_char_cntrs_cntr_idx.extend(cntr["cntr_idx"] for cntr in id_char_cntrs)

        # print(f'{self.img["fname"]} 染色体编号轮廓索引:{id_char_cntrs_cntr_idx}')

        # 获取除了染色体编号字符轮廓之外的轮廓
        left_cntr_dicts = [cntr for cntr in self.cntr_dicts if cntr["cntr_idx"] not in id_char_cntrs_cntr_idx]
        ...
        for key in left_cntr_dicts_orgby_cy:
            left_cntr_dicts_orgby_cy[key] = sorted(left_cntr_dicts_orgby_cy[key], key=lambda item: item["cx"])
        ...
        for cy_key, same_cy_left_cntr_dicts in left_cntr_dicts_orgby_cy.items():
            for left_cntr in same_cy_left_cntr_dicts:
                ...
                for id_cntr in self.id_cntr_dicts_orgby_cy[cy_key]:
                    ...
                    left_cntr_bc_point = np.array(left_cntr["bc_point"], dtype=np.int32)
        ...
        for same_cy_left_cntr_dicts in left_cntr_dicts_orgby_cy.values():
            ...
            for chromo_id in same_cy_unq_chromo_id_list:
                same_chromo_id_cnts_dict = [
                    the_dict for the_dict in same_cy_left_cntr_dicts if the_dict["chromo_id"] == chromo_id
                ]
                ...
                small_cntr_dicts = [
                    cntr
                    for cntr in same_chromo_id_cnts_dict
                    if cntr["area"] / max_area < CHROMO_SPLINTER_AREA_RATIO_UPPER
                ]
                main_cntr_dicts = [
                    cntr
                    for cntr in same_chromo_id_cnts_dict
                ...
                for small_cntr_dict in small_cntr_dicts:
                    ...
                    for main_cntr_dict in main_cntr_dicts:
                        distance, cntr_npi1, cntr_npi2 = get_distance_between_two_contours(
                            small_cntr_dict["cntr"], main_cntr_dict["cntr"]
                        )
                        if distance < min_distance:
                            min_distance = distance
                    ...
                    # 合并小碎片和主干轮廓
                    merged_cnt = merge_two_contours_by_npi(
                        small_cntr_dict["cntr"], nearest_main_cntr_dict["cntr"], small_cntr_npi, main_cntr_npi
                    )
        ...
        for same_cy_left_cntr_dicts in left_cntr_dicts_orgby_cy.values():
            cnts_idx_need_delete = [
                idx for idx, cntr in enumerate(same_cy_left_cntr_dicts) if cntr["cntr_idx"] in small_cnts_need_delete
            ]
        ...
        # 将chromo_cntr_dicts_orgby_cy中的染色体轮廓dict全部展开到chromo_cntr_dicts中
        self.chromo_cntr_dicts = []
        for chromo_cntr_dict_list in left_cntr_dicts_orgby_cy.values():
            for chromo_cntr_dict in chromo_cntr_dict_list:
                self.chromo_cntr_dicts.append(chromo_cntr_dict)
    ...
    def _gather_contours_dict(self, idx, contour):
        ...
        contours, _ = find_external_contours_en(wbg_cropped, bin_thresh=253, bin_type=cv2.THRESH_BINARY_INV, bin_thresh_adjustment=0)
        # contours, _ = find_external_contours(gray_cropped, bin_thresh=252)
