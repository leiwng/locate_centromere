...
def cv_imread(file_path, flags=cv2.IMREAD_UNCHANGED):
    ...
    """
    return cv2.imdecode(np.fromfile(file_path, dtype=np.uint8), flags=flags)  # 前值 cv2.IMREAD_COLOR
...
def find_contours_en(
    ...
    if bin_thresh is not None and bin_thresh >= 0:
        ...
        raise ValueError("func:find_contours_en,bin_thresh参数错误")

    # 找轮廓
    contours, hierarchy = cv2.findContours(bin_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    return contours, hierarchy, dst_bin_thresh,
    ### END of find_contours_en


def get_proper_threshold_4_contours(img):

    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if len(img.shape) == 3 else img
    thresh, _ = cv2.threshold(gray_img, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_TRIANGLE)
    return int(thresh)
